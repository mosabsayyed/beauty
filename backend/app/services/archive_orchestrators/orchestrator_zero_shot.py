"""
Zero-Shot Orchestrator - Cognitive Digital Twin (v2.0)
...
"""
    ...
    def _normalize_response(self, resp: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 4) Clean up excessive line breaks & Fix Markdown Bold (User Request)
        # Collapse 3 or more newlines (possibly with whitespace) into 2
        if isinstance(result.get("answer"), str):
            result["answer"] = re.sub(r'(\n\s*){3,}', '\n\n', result["answer"])
            # Fix mixed Markdown bold syntax (**text**) to HTML (<b>text</b>)
            result["answer"] = re.sub(r'\*\*(.*?)\*\*', r'<b>\1</b>', result["answer"])

        # 5) Server-Side HTML Detection (The Fix)
        ...
        # Improved regex to catch HTML even if wrapped in markdown blocks or with leading text
        # Relaxed check: also look for common block-level HTML tags if the user asked for HTML
        if re.search(r"<!doctype html>|<html\b|<h[1-6]\b|<table\b|<div\b", answer_str, re.IGNORECASE) or "</html>" in answer_str:
            # Clean up markdown code blocks if present
            # Robust extraction: Find the first code block containing HTML
