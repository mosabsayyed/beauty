"""
Zero-Shot Orchestrator - Cognitive Digital Twin (v3.5)
...
#### NODES AND LEVEL MEANINGS (FULL DICTIONARY)
...
5.  *Temporal Logic*: Queries must explicitly filter nodes by `year` (e.g., `WHERE n.year = 2026`).
6.  *Property mappings:
...
2.  **Continuity Strategy (Keyset Pagination):**
    ...
    def _normalize_response(self, resp: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result["answer"]:
            try:
                ...
                end = result["answer"].rfind('}')
                
                if start != -1 and end != -1:
                    json_str = result["answer"][start:end+1]
                ...
                # If parsing fails, we leave result["answer"] as the raw text
