...
class SupabaseClient:
    ...
    async def connect(self) -> None:
        ...
        if not self.client:
            # We still create client object so external code accessing .client (like setup.py) works
            try:
    ...
    async def _run_http(self, endpoint: str, method: str = "GET", params: Dict[str, str] = None, body: Any = None) -> Any:
        ...
        try:
            ...
            if response.status_code >= 400:
                # Suppress 404s (not found) as they're expected during concurrent operations
    ...
    async def table_count(self, table: str, filters: Optional[Dict[str, Any]] = None) -> int:
        """Count rows using async HTTP client (non-blocking)."""
        ...
        # Build query string
        query_parts = ["select=*", "count=exact", "limit=1"]
        if filters:
             for k, v in filters.items():
                query_parts.append(f"{k}=eq.{v}")
        full_url = f"{self.url}/rest/v1/{table}?" + "&".join(query_parts)
        ...
        try:
            response = await self._http_client.head(full_url, headers=headers)
            ...
            logger.error(f"Error executing count request: {e}")
            return 0

    async def rpc(self, function_name: str, params: Optional[Dict[str, Any]] = None) -> Any:
        """Execute RPC using async HTTP client (non-blocking)."""
        ...
        try:
            response = await self._http_client.post(full_url, headers=headers, json=params)
            if response.status_code >= 400:
                logger.error(f"HTTP {response.status_code}: {response.text}")
                return None
            ...
            logger.error(f"Error executing RPC: {e}")
            return None
