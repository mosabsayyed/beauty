...
def clean_html_content(content, title, lang='en'):
    ...
    # SKIP AGGRESSIVE TABLE UNWRAPPING TO AVOID TRUNCATION
    ...
    # css_path is now hardcoded in TEMPLATE_START
    
    html = TEMPLATE_START.format(lang=lang, dir_attr=dir_attr, title=title)
    html += body_content
    html += TEMPLATE_END
...
def process_all_files():
    # Get all HTML files in knowledge dir
    files = [f for f in os.listdir(BASE_DIR) if f.endswith('.html') and re.match(r'\d+\.\d+\.html', f)]
    
    for filename in files:
        # Skip 1.4.html as it was manually restored and user requested no script touch
        if filename == '1.4.html':
