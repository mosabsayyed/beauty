...
# run_dev_full_frontend2.sh - start MCP, ngrok, backend and frontend2 in background and record PIDs
...
mkdir -p "$ROOT_DIR/frontend2/logs"

PID_FILE="$ROOT_DIR/.dev_pids_frontend2"
rm -f "$PID_FILE"
...
echo "Starting frontend2 dev server (npm run dev) in background..."
nohup npm --prefix "$ROOT_DIR/frontend2" run dev 2>&1 | python3 -u -c "import sys,time\nfor l in sys.stdin:\n  sys.stdout.write(time.strftime('[%Y-%m-%d %H:%M:%S] ')+l)\n  sys.stdout.flush()" >> "$ROOT_DIR/frontend2/logs/frontend2.log" 2>/dev/null &
echo $! >> "$PID_FILE"
...
if [ "$NO_TAIL" -eq 1 ]; then
  ...
  echo "Tailing important logs (mcp / backend / frontend2); press Ctrl-C to stop tailing"
  tail -n 40 -f "$ROOT_DIR/backend/logs/mcp_server.log" "$ROOT_DIR/logs/backend_start.log" "$ROOT_DIR/frontend2/logs/frontend2.log"
fi
