...
const CanvasManager: React.FC = () => {
  ...
  useEffect(() => {
    // Load recent artifacts (stubbed)
    setArtifacts([]);
  }, []);

  // Listen for the global toggleCanvas event dispatched by Header (quick wiring)
  useEffect(() => {
    const handler = () => {
      // toggle between hidden and collapsed for quick header control
  ...
  // Sample chart artifact for demonstration
  useEffect(() => {
        ...
        <div className="canvas-content">
          ...
          <div className="artifact-body">
            {currentArtifact.artifact_type === 'CHART' ? (
              <React.Suspense fallback={<div className="chart-placeholder">Loading chart...</div>}>
                {/* Dynamically import ChartRenderer to avoid circular dependency */}
                {(() => {
                  const ChartRenderer = require('./ChartRenderer').default;
                  return <ChartRenderer artifact={currentArtifact} />;
                })()}
              </React.Suspense>
            ) : (
