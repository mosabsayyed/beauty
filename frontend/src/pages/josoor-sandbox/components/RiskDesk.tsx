...
const RiskDesk: React.FC<RiskDeskProps> = ({ quarter, year }) => {
    const { language } = useLanguage();
    const isDark = true;
    const [localYear, setLocalYear] = useState<string>('2025');
    const [selectedView, setSelectedView] = useState<'overview' | 'detail'>('overview');
    const [selectedRiskId, setSelectedRiskId] = useState<string | null>(null);

    // Use prop year if provided (Global Filter), otherwise local state
    const effectiveYear = year || localYear;

    // Fetch Dashboard Data (reusing the same endpoint as ControlTower for consistent data)
    const { data, isLoading, error, refetch } = useQuery({
        queryKey: ['dashboardMetrics', effectiveYear],
        queryFn: async () => {
            const response = await fetch(`/api/dashboard/metrics?year=${effectiveYear}`);
            if (!response.ok) {
